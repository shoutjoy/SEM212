...
t_test_report <- function(data, iv=NULL, dv = NULL, type = "all") {
  ...
  if (var_test_result$p.value < 0.05) {
    ...
    t_test_result_tibble <- tibble(
      ...
      sig  = dplyr::case_when( p.value < 0.001 ~ "***",  p.value < 0.01 ~ "**",  p.value < 0.05 ~ "*",   TRUE ~ "ns" ),
      test_type ="welch's t.test"
    )
    t_test_report <- paste(
      "The t-test between independent variable '", iv, "' and dependent variable '", dv, "' was",
      ifelse(t_test_result$p.value < 0.05, "statistically significant, t(", "not statistically significant, t("),
      round(t_test_result$statistic, 2), ",",
      round(t_test_result$p.value, 2), ".")
  } else {
    t_test_result <- t.test(dv_value ~ iv_value)
    t_test_result_tibble <- tibble(
      IV = iv,
      DV = dv,
      df = t_test_result$parameter,
    ...
    t_test_report <- paste(
      "The t-test between independent variable '", iv, "' and dependent variable '", dv, "' was not statistically significant, t( 98 ) =  0.97 , p > .05.")
  }

  # box plot output
  gg <- ggplot(data, aes(x = factor(iv_value),
                         y = dv_value,
                         group = iv_value)) +
    geom_boxplot() +
    ggtitle(t_test_report)
